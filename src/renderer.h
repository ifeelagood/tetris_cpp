...
private:
    ...
    void drawTiles(std::vector<std::vector<int>> const &tiles, unsigned int level);

    void generateTexture();
    void drawTexture(unsigned int w, unsigned int h);

    void drawCharNumber(unsigned int num, unsigned int x1, unsigned int y1);
    void drawCharAlpha(unsigned char c, unsigned int x1, unsigned int y1);
...
public:
    ...
    void render(unsigned int windowWidth, unsigned int windowHeight, std::vector<std::vector<int>> const &pile, Piece const &piece, unsigned int level, unsigned int score, unsigned int topScore, unsigned int linesCleared);
